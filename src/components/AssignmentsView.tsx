...
export default function AssignmentsView({ ensembleId, isDirector, currentUserId, roster }: AssignmentsViewProps) {
  ...
  const fetchData = async () => {
    setLoading(true)
    const { data: asmData } = await supabase.from('assignments').select('*').eq('ensemble_id', ensembleId).order('created_at', { ascending: false })
    if (asmData) setAssignments(asmData)

    const { data: subData } = await supabase.from('assignment_submissions').select('*, assignments!inner(ensemble_id)').eq('assignments.ensemble_id', ensembleId)
  ...
  const handleSaveFeedback = async (submissionId: string, notes: string) => {
    const { error } = await supabase.from('assignment_submissions').update({ teacher_notes: notes }).eq('id', submissionId)
    if (error) alert(error.message)
    else setSubmissions(submissions.map(s => s.id === submissionId ? { ...s, teacher_notes: notes } : s))
  }
  ...
  const submitRecording = async () => {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('audio_submissions').upload(fileName, audioBlob, {
  ...
  if (selectedAssignment) {
    ...
    return (
      <div className="animate-in slide-in-from-right-8 duration-300">
        <button onClick={() => { setSelectedAssignment(null); setAudioBlob(null); setAudioUrl(null) }} className="mb-6 text-sm text-slate-400 hover:text-white transition flex items-center gap-2 font-medium cursor-pointer">
          ← Back to Assignments
        </button>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2rem] p-8 shadow-2xl">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6">
              ...
              {mySubmission ? (
                <div className="space-y-6">
                  ...
                  {mySubmission.teacher_notes && (
                    <div className="bg-indigo-500/10 border border-indigo-500/20 p-5 rounded-xl">
                      ...
                      <p className="text-sm text-indigo-100 italic">"{mySubmission.teacher_notes}"</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-8">
                  {audioUrl ? (
                    <div className="w-full max-w-md space-y-6 text-center">
                      ...
                      <div className="flex gap-3">
                        <button onClick={() => { setAudioBlob(null); setAudioUrl(null) }} className="w-1/2 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold transition cursor-pointer">
                          Retake
                        </button>
                    ...
                    <button 
                      ...
                      className={`relative w-24 h-24 rounded-full flex items-center justify-center shadow-2xl transition-all cursor-pointer ${
                        isRecording 
                          ? 'bg-rose-500 hover:bg-rose-400 shadow-rose-500/50 scale-110 animate-pulse' 
                  ...
                  {!audioUrl && <p className="text-slate-400 text-sm font-medium mt-6">{isRecording ? 'Recording in progress... Click to stop.' : 'Click to start recording your playing test.'}</p>}
                </div>
              )}
            </div>
          )}
          ...
          {isDirector && (
            <div>
              <h4 className="text-xs uppercase tracking-widest font-bold text-slate-500 mb-4">Student Submissions ({assignmentSubmissions.length} / {roster.filter(r=>r.role === 'member').length})</h4>
              ...
              {assignmentSubmissions.length === 0 ? (
                ...
                <div className="space-y-4">
                  {assignmentSubmissions.map(sub => {
                    const student = roster.find(r => r.user_id === sub.user_id)
                    return (
                      <div key={sub.id} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-4">
                        ...
                        <div className="pt-2">
                          <label className="text-[10px] uppercase font-bold text-slate-500 tracking-wider mb-1 block">Your Feedback</label>
                          <div className="flex gap-2">
                            <input 
                              type="text" 
                              placeholder="Leave a note or grade..." 
                              defaultValue={sub.teacher_notes || ''} 
                              onBlur={e => handleSaveFeedback(sub.id, e.target.value)}
      ...
      {isDirector && isAdding && (
        <form onSubmit={handleCreateAssignment} className="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Cello Excerpt - Measure 40-80" value={newTitle} onChange={e => setNewTitle(e.target.value)} required className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-sm text-white outline-none focus:border-indigo-500" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {assignments.map(asm => {
            ...
            return (
              <div key={asm.id} onClick={() => setSelectedAssignment(asm)} className="p-6 rounded-2xl bg-slate-900 border border-slate-800 hover:border-indigo-500/50 hover:bg-slate-800/80 transition cursor-pointer flex flex-col justify-between space-y-4">
                <div>
                ...
                {isDirector && (
                  <div className="pt-4 border-t border-slate-800">
                    <p className="text-xs text-slate-400 font-semibold">{submissionCount} / {roster.filter(r=>r.role==='member').length} Submissions</p>
                  </div>
                )}
              </div>
            )
